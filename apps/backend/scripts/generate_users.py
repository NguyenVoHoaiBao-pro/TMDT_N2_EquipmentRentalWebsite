...
# Predefined lists of Vietnamese first names and last names for realistic user generation
FIRST_NAMES_MALE = [
...
def generate_users_sql(users):
    ...
    values = []
    for user in users:
        value = f"""    ('{user['username']}', '{user['password']}', '{user['full_name']}', '{user['email']}', '{user['phone']}', '{user['id_card']}', {user['trust_score']}, {user['enabled']}, NOW(), NOW())"""
        values.append(value)
...
"""
    ...
    # Output to database/seed_users.sql
